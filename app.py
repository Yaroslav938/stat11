...
st.set_page_config(page_title="🔬 StatPack OmniLab v16", layout="wide", page_icon="📈")
...
st.title("📈 StatPack OmniLab v16: Universal & Growth Edition")
st.markdown("*Мульти-аналитическая станция для статистики, визуализации и расчета автоматизированных алгоритмов эффективности.*")
...
if uploaded_files:
    ...
    if is_ready and wide_df is not None and not wide_df.empty:
        ...
        with t7:
            st.markdown("### 🧮 Автоматизированный расчет биологических параметров (Рост и Индексы)")
            st.caption("Позволяет рассчитать абсолютный, среднесуточный и относительный прирост, SGR, упитанность по Фультону и кормовой коэффициент (FCR). Идеально для аквакультуры и биологии.")
            
            c_calc1, c_calc2, c_calc3 = st.columns(3)
            
            # Настройка параметров в зависимости от формата данных
            if has_time:
                with c_calc1:
                    calc_mass = st.selectbox("Признак Массы (Вес):", long_df["Признак"].unique())
                    calc_len = st.selectbox("Признак Длины (для Фультона):", ["Не учитывать"] + list(long_df["Признак"].unique()))
                with c_calc2:
                ...
                with c_calc3:
                    ...
                    feed = st.number_input("Израсходовано корма (г) на 1 особь:", min_value=0.0, value=0.0, step=10.0, help="Оставьте 0, если расчет FCR не нужен.")
            else:
                with c_calc1:
                ...
                with c_calc3:
                    days = st.number_input("Продолжительность (дней):", min_value=1, value=30)
                    feed = st.number_input("Израсходовано корма (г) на 1 особь:", min_value=0.0, value=0.0, step=10.0)

            st.markdown("---")
            
            # Логика математического расчета
            calc_results = []
            ...
            for grp in unique_groups:
                try:
                    if has_time:
                        ...
                        W_n = df_grp[(df_grp["Период"] == p_start) & (df_grp["Признак"] == calc_mass)]["Значение"].mean()
                        W_k = df_grp[(df_grp["Период"] == p_end) & (df_grp["Признак"] == calc_mass)]["Значение"].mean()
                        
                        L_n, L_k = 0, 0
                        if calc_len != "Не учитывать":
                            L_n = df_grp[(df_grp["Период"] == p_start) & (df_grp["Признак"] == calc_len)]["Значение"].mean()
                            L_k = df_grp[(df_grp["Период"] == p_end) & (df_grp["Признак"] == calc_len)]["Значение"].mean()
                    else:
                    ...
                    K_n = (W_n / (L_n ** 3)) * 100 if L_n > 0 else np.nan
                    K_k = (W_k / (L_k ** 3)) * 100 if L_k > 0 else np.nan
                    ...
                    calc_results.append({
                        ...
                        "Удельная скорость (Cw), %": round(Cw, 3),
            ...
            if calc_results:
                ...
                st.warning("Нет данных для расчета по выбранным параметрам. Проверьте правильность выбора столбцов Массы.")
    ...
    st.markdown("""
    ### 🔬 Добро пожаловать в StatPack OmniLab (v16)
    **Новое в текущей версии:**
    * 🧮 **Модуль биологических индексов:** Добавлена новая вкладка для автоматизированного расчета абсолютного/среднесуточного прироста, удельной скорости роста (SGR), коэффициента упитанности по Фультону и индекса конверсии корма (FCR). Алгоритм полностью повторяет логику сложных Excel-шаблонов.
    * 📂 **Мульти-загрузка файлов:** Идеально для сравнения контроля и опыта во времени.
    """)